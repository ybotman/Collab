...
## 2026-06-29 — Never hand Toby `!`-prefixed commands for his zsh terminal (caused a secret leak)
**Lesson:** The `!` prefix means "run in-session" only at the **Claude Code prompt**. Toby often pastes into his **real zsh shell**, where a leading `!` triggers **history expansion** — which mangled two commands into one and **printed an Azure OpenAI API key in plaintext** (leak). 
**Why it matters:** a mangled secret-handling command leaks the secret to terminal scrollback + chat transcript.
**How to apply:** When giving Toby a terminal command, give it **plain, no `!`**. For secrets, always pipe through a subshell so the value is never echoed (`--value "$(az ... -o tsv)" >/dev/null`), and prefer a **script file run as `bash <path>`** over a pasted multi-part command (no history-search/`^R` mangle risk). If a secret ever prints, **rotate it immediately**.
